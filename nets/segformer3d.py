...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        spatial_shape = tuple(patches.shape[2:])  # (D, H, W)
    ...
    def forward(self, x, spatial_shape=None):
        ...
        if self.sr_ratio > 1:
            if spatial_shape is None:
                ...
                d, h, w = [int(s) for s in spatial_shape]
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, spatial_shape=None):
        ...
        x = x + self.mlp(self.norm2(x), spatial_shape=spatial_shape)
...
class MixVisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        for blk in self.tf_block1:
            x, _ = blk(x, spatial_shape=(d, h, w))
        x = self.norm1(x)
        x = x.reshape(x.shape[0], d, h, w, -1).permute(0, 4, 1, 2, 3).contiguous()
        out.append(x)
...
class _MLP(nn.Module):
    ...
    def forward(self, x, spatial_shape=None):
        x = self.fc1(x)
        x = self.dwconv(x, spatial_shape=spatial_shape)
        x = self.act_fn(x)
        x = self.dropout(x)
        x = self.fc2(x)
        x = self.dropout(x)

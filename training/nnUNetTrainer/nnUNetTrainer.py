...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        self.my_init_kwargs = {}
        for k, p in inspect.signature(self.__init__).parameters.items():
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        # Unwrap DDP before inference: the predictor does single-GPU sliding-window
        # inference and should use the raw model.  Passing a DDP-wrapped model to the
        # predictor causes subtle issues — the DDP forward hook runs even in eval mode
        # and can interfere with test-time mirror augmentation (torch.flip receives a
        # list-of-tensors instead of a single tensor when the adapter's DS flag wasn't
        # properly toggled).
        inference_network = self.network.module if self.is_ddp else self.network
        predictor.manual_initialization(inference_network, self.plans_manager, self.configuration_manager, None,
